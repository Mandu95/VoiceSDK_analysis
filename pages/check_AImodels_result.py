...
def calculate_metrics(df, origin_col, target_col):
    ...
    cer_value = round(result_df.apply(lambda row: cer(
        row[origin_col], row[target_col]), axis=1).mean(), 3)
    wer_value = round(result_df.apply(lambda row: wer(
        row[origin_col], row[target_col]), axis=1).mean(), 3)
...
def compare_from_file():
    ...
    if uploaded_file is not None:
        ...
        if len(columns) >= 2:
            ...
            tab_titles = ["Overview",
                          f"Comparison: {origin_col}-{target1_col}"]
            if target2_col:
            ...
            with tabs[0]:
                ...
                mean_ratio1, cer_value1, wer_value1, comparer1 = calculate_metrics(
                    df, origin_col, target1_col)
                metrics_data = {
                ...
                if target2_col:
                    mean_ratio2, cer_value2, wer_value2, comparer2 = calculate_metrics(
                        df, origin_col, target2_col)
                    metrics_data['Comparison'].append(
                        f"{origin_col}-{target2_col}")
...
page = st.sidebar.selectbox(
    "Choose a page:", ["Compare from uploaded file", "Compare by entering text"])

if page == "Compare from uploaded file":
    st.title('Text Compare')
    st.write('이 페이지는 음성인식 모델의 성능을 검증하는 페이지 입니다.')
