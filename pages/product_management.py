...
def show_product_management():
    df = product_management.drop(columns=['개발언어', '계약관리'])  # 필요한 열만 남기고 제거
    ...
    if tab_label == "전체":
        filtered_data = product_management.drop(columns=['개발언어', '계약관리'])
    else:
        filtered_data = product_management[product_management['연관 제품'].apply(
            lambda x: tab_label in x if isinstance(x, list) else tab_label == x)]
        filtered_data = filtered_data.drop(columns=['개발언어', '계약관리'])
