...
def Notion_properties_URL(data):

    notion_data_count = len(data)
    print("Notion DB 개수 : ", notion_data_count)

    # 빈 이중배열 선언  >
    empty_array = [[] for _ in range(notion_data_count)]
...
def get_value_from_property(property):  # notion properties 중 롤업 속성에 대한 별도 함수
...
def get_rollup_value(property):  # Rollup 속성에서 값을 추출하는 함수
    ...
    if property['rollup']['type'] == "array":
        ...
        return safe_get(property, ['rollup', 'date', 'start'])


def safe_get(d, keys, default=None):  # 파싱할 때 에러 발생 시 예외처리하고 파싱 이어가는 함수
...
def find_same_data(list_data):  # 리스트에서 중복되지 않은 데이터 추출 함수
...
def normalize_column_lists(df):  # 데이터프레임의 각 열을 순회하면서 리스트 형태의 값들을 쉼표로 구분된 문자열로 변환
...
def Change_data(df, column_name=None):  # 데이터프레임의 특정 열에 대해 타입 변환 (표준시, 금액단위)

    df = Change_date_iso8601(df, column_name)
    df = Change_data_type(df)

    return df


def Change_data_type(df):  # 금액 단위로 변환 / 숫자 텍스트로 변환
    # 금액 단위로 변환되는 열 목록
