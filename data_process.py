...
def update_notion_data():
    ...
    # 표 View를 위한 속성 추출
    row_name = function.df_col(product_management)

    # 값 추출
    product_management = function.extract_data(product_management, row_name)

    # 두 번째 DB의 데이터 사용
    contract_management_db = data[1]
    product_management = function.change_contract_data(
        contract_management_db, product_management)

    etc_document_db = data[2]

    product_management = function.change_etc_docu_data(
    ...
    # 표 View를 위한 속성 추출
    etc_document_row_name = function.df_col(etc_document)
    # 값 추출
    etc_document = function.extract_data(etc_document, etc_document_row_name)
    # 두 번째 DB의 데이터 사용
    product_management_db = data[0]
    etc_document = function.change_company_name_data(
    ...
    contract_management_row_name = function.df_col(contract_management)

    # 값 추출
    contract_management = function.extract_data(
    ...
    contract_management = function.change_data_type(
        product_management_db, contract_management)


# 매일 오전 7시, 오후 12시에 작업 스케줄링
schedule.every().day.at("07:00").do(update_notion_data)
schedule.every().day.at("12:00").do(update_notion_data)
